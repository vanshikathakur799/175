...
AFRAME.registerComponent('markerhandler', {
    ...
    getDistance: function (elA, elB) {
      ...
      isModelPresentArray: function(arr, val){
        for (var i of arr) {
            if(i.model_name === val) {
                return true
            }
      ...
      tick: async function() {
        if(modelList.length > 1){
            var isBaseModelPresent = this.isModelPresentArray(modelList, 'base')
            ...
            if(!isBaseModelPresent) {
                ...
                this.placeTheModel("tree_1", models)
            }
        }
      },

      placeTheModel: function(modelName, models) {
        var isListContainModel = this.isModelPresentArray(modelList, modelName)
